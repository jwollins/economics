...
yield_wbeans_ca <- rnorm(n_sim, mean_yield_wbeans_ca, sd_yield_wbeans_ca)
...
expen_wbeans_ca <- rnorm(
  n = n_sim, 
  mean = wbeans_expenditure_mean, 
  sd = wbeans_expenditure_sd)
...
expen_sbarley_ca <- rnorm(
  n = n_sim, 
  mean = sbarley_expenditure_mean, 
  sd = sbarley_expenditure_sd)
...
expen_peas_ca <- rnorm(
  n = n_sim, 
  mean = peas_expenditure_mean, 
  sd = peas_expenditure_sd)
...
simulate_yield_trend <- function(initial_yield, drift, volatility, years = 12) {
...
# Apply to each crop
yield_wheat_trend <- simulate_yield_trend(mean_yield_wheat_con, yield_drift, yield_volatility)
yield_wbarley_trend <- simulate_yield_trend(mean_yield_wbarley_con, yield_drift, yield_volatility)
yield_wosr_trend <- simulate_yield_trend(mean_yield_wosr_con, yield_drift, yield_volatility)

yield_wbeans_trend <- simulate_yield_trend(mean_yield_wbeans_ca, yield_drift, yield_volatility)
yield_sbarley_trend <- simulate_yield_trend(mean_yield_sbarley_ca, yield_drift, yield_volatility)
yield_fpeas_trend <- simulate_yield_trend(mean_yield_peas_ca, yield_drift, yield_volatility)
...
revenue_conventional <- ifelse(
  crop_sequence_conventional == "Winter Wheat", yield_wheat_trend[start_years_conventional] * price_wheat_trend[start_years_conventional],
  ifelse(crop_sequence_conventional == "Winter Barley", yield_wbarley_trend[start_years_conventional] * price_wbarley_trend[start_years_conventional],
         yield_wosr_trend[start_years_conventional] * price_wosr_trend[start_years_conventional])  # Oilseed Rape as default
)

revenue_conservation <- ifelse(
  crop_sequence_conservation == "Winter Beans", yield_wbeans_trend[start_years_conservation] * price_wbeans_trend[start_years_conservation],
  ifelse(crop_sequence_conservation == "Winter Wheat", yield_wheat_trend[start_years_conservation] * price_wheat_trend[start_years_conservation],
         ifelse(crop_sequence_conservation == "Spring Barley", yield_sbarley_trend[start_years_conservation] * price_sbarley_trend[start_years_conservation],
                ifelse(crop_sequence_conservation == "Oilseed Rape", yield_wosr_trend[start_years_conservation] * price_wosr_trend[start_years_conservation],
                       ifelse(crop_sequence_conservation == "Feed Peas", yield_fpeas_trend[start_years_conservation] * price_fpeas_trend[start_years_conservation], 0)))))  # Default: no revenue for cover crops
...
price_data <- data.frame(
  Year = rep(1:12, 3),
  Price = c(price_wheat_trend, price_wbarley_trend, price_wosr_trend),
  Crop = rep(c("Winter Wheat", "Winter Barley", "Oilseed Rape"), each = 12)
)

ggplot(price_data, aes(x = Year, y = Price, color = Crop)) +
  geom_line(size = 1.2) +
  labs(title = "Simulated Crop Price Trends Over 12 Years", y = "Price (currency/unit)", x = "Year") +
  theme_minimal()


yield_data <- data.frame(
  Year = rep(1:12, 3),
  Yield = c(yield_wheat_trend, yield_wbarley_trend, yield_wosr_trend),
  Crop = rep(c("Winter Wheat", "Winter Barley", "Oilseed Rape"), each = 12)
)

ggplot(yield_data, aes(x = Year, y = Yield, color = Crop)) +
  geom_line(size = 1.2) +
  labs(title = "Simulated Yield Trends Over 12 Years", y = "Yield (kg/ha)", x = "Year") +
  theme_minimal()
...
# ~ Simulating Climate Shocks ####

# Probability of a climate shock in each year (increasing over time)
climate_shock_prob <- seq(0.05, 0.20, length.out = 12)  # 5% in Year 1, 20% in Year 12

# Define severity of yield reduction (10% to 30%)
climate_shock_severity <- function() runif(1, min = 0.1, max = 0.3)  
...
yield_wheat_trend <- apply_climate_shocks(yield_wheat_trend)
yield_wbarley_trend <- apply_climate_shocks(yield_wbarley_trend)
yield_wosr_trend <- apply_climate_shocks(yield_wosr_trend)

yield_wbeans_trend <- apply_climate_shocks(yield_wbeans_trend)
yield_sbarley_trend <- apply_climate_shocks(yield_sbarley_trend)
yield_fpeas_trend <- apply_climate_shocks(yield_fpeas_trend)
...
# ~~~~~~~~~~~~####
# Cost Inflation on Expenditure ####


# Define annual expenditure inflation rate (2-4%)
expenditure_inflation <- 0.03  
...
# Apply to each crop
expen_wheat_trend <- simulate_expenditure_trend(wheat_expenditure_mean, expenditure_inflation, expenditure_volatility)
expen_wbarley_trend <- simulate_expenditure_trend(wbarley_expenditure_mean, expenditure_inflation, expenditure_volatility)
expen_wosr_trend <- simulate_expenditure_trend(wosr_expenditure_mean, expenditure_inflation, expenditure_volatility)

expen_wbeans_trend <- simulate_expenditure_trend(wbeans_expenditure_mean, expenditure_inflation, expenditure_volatility)
expen_sbarley_trend <- simulate_expenditure_trend(sbarley_expenditure_mean, expenditure_inflation, expenditure_volatility)
expen_fpeas_trend <- simulate_expenditure_trend(peas_expenditure_mean, expenditure_inflation, expenditure_volatility)
