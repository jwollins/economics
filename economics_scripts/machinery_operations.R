...
TFC_sum <- dat %>%
  ...
  summarise( 
    ...
    diesel_consum_kg_ha_sum = sum(diesel_consum_kg_ha, na.rm = TRUE)
  )
...
tfc_plot <- 
  ...
  labs(
    x = "Year",
    y = "TFC",
    subtitle = "Theoretical field capacity (TFC)") +
  facet_wrap(~ year, ncol = 3) +
  theme_bw() +
  theme(strip.text.x = element_text(size = 12, 
                                    # color = "black", 
...
diesel_kg_ha_plot <- 
  ...
  labs(
    x = "Year",
    y = expression("Theoretical diesel consumption (kg ha"^{-1}*")"),
    subtitle = expression("Theoretical diesel consumption (kg ha"^{-1}*")")
  ) +
  facet_wrap(~ year, ncol = 3) +
  theme_bw() +
  theme(strip.text.x = element_text(size = 12, 
...
ggarrange(opp_n_plot, time_per_ha_plot, diesel_kg_ha_plot, 
          ncol = 3, 
          common.legend = TRUE, 
          legend = "bottom")
